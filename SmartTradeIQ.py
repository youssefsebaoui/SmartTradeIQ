...
rsi_oversold = 30
#============================================================================================================
# Fonction pour détecter les pics de volume
def detect_volume_spike(candles, volume_threshold):
    volumes = [candle['volume'] for candle in candles]
    average_volume = np.mean(volumes)
    latest_volume = volumes[-1]

    if latest_volume > volume_threshold * average_volume:
        return 'volume_spike'
    else:
        return None
...
# Fonction pour détecter le range avec confirmation par l'ADX
def detect_range_with_adx(candles):
    highs = np.array([candle['max'] for candle in candles])
    lows = np.array([candle['min'] for candle in candles])
    closes = np.array([candle['close'] for candle in candles])
    
    if len(closes) < 14:  # S'assurer d'avoir assez de données pour calculer l'ADX
        return None
    ...
    if last_adx >= 25:
        return 'strong'  # Tendance forte
    else:
        return 'weak'  # Tendance faible
#============================================================================================================
    ...
    while int(time.localtime().tm_sec % 60) < 1:
        for asset in sorted_keys:
            if asset in ACTIVES :
                ...
                candles = api.get_candles(asset, candle_timeframe, 60, time.time())
                ...
                print('signal: ',signal)
                adx_signal = detect_range_with_adx(candles)
                print('ADX signal: ', adx_signal)
                # Détection du pic de volume
                signal_volume = detect_volume_spike(candles, 2.0)  # Ajustez le seuil de volume selon vos besoins
                print('signal_volume: ',signal_volume)
                signal_volume = 'volume_spike';
                if signal_volume == 'volume_spike':
                    if signal == 'oversold' and latest_close <= support and adx_signal == 'strong' and par != asset:
                        ...
                        break
                    elif signal == 'overbought' and latest_close >= resistance  and adx_signal == 'strong' and par != asset:
